...
class MainDecoder(nn.Module):
    def __init__(self, upscale, conv_in_ch):
        super(MainDecoder, self).__init__()
        self.upsample = upsample(conv_in_ch, 1, upscale=upscale)
...
class DropOutDecoder(nn.Module):
    def __init__(self, upscale, conv_in_ch, drop_rate=0.3, spatial_dropout=True):
...
class FeatureNoiseDecoder(nn.Module):
    def __init__(self, upscale, conv_in_ch, uniform_range=0.3):
...
class VATDecoder(nn.Module):
    def __init__(self, upscale, conv_in_ch, xi=1e-1, eps=10.0, iterations=1):
...
class CutOutDecoder(nn.Module):
    def __init__(self, upscale, conv_in_ch, drop_rate=0.3, spatial_dropout=True, erase=0.4):
...
class ObjectMaskingDecoder(nn.Module):
    ...
    def forward(self, x, pred=None):
        ...
        return [x_masked_obj, self.sig(x_masked_obj)]
